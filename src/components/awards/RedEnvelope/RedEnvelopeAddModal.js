    ...
    return (
      <Modal title="添加红包">
        <form method="post" onSubmit={this.props.submit}>
          ...
          <hr style={{ borderStyle: 'dashed' }} />
          <Textarea labelName="短信模板" defaultValue={item.mail || getConfig('templateTypes', this.state.type)} name="mail" />
          <Textarea labelName="站内信模板" name="mail" defaultValue={item.message} />
          <Submit />
        </form>
      </Modal>
    );
