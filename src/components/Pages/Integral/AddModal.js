    ...
    return (
      <Modal title={this.props.update ? '编辑商品' : '添加商品'}>
        <form onSubmit={this.props.submit}>
          ...
          <Select labelName="奖品类型" name="award_type" options={getConfig('awardTypes')} defaultValue={this.props.item.award_type || 1} />
          <Input labelName="奖品ID" name="award_id" type="number" required defaultValue={this.props.item.award_id || ''} />
          <DateTimeInput required limit labelName="开始时间" name="start_time" defaultValue={this.props.item.start_time} />
          <DateTimeInput required limit labelName="结束时间" name="end_time" defaultValue={this.props.item.end_time} />
